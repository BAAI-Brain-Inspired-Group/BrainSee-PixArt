...
class PixArt_C_Ext(PixArt_C):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.lay_emb = nn.Embedding(6, 1152)
        nn.init.trunc_normal_(self.lay_emb.weight.data, mean=0, std=init_std)

        # self.l_prompt = 30
        # self.prompts1 = nn.ParameterList([
        #     nn.Parameter(torch.randn(1, self.l_prompt, 1152)) 
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, img_o=None, **kwargs):
        ...
        rc = int(1)
        f_ori = self.hypercolumn.to_embedding(img_o,rc)
        # layer_C = (torch.ones((B, 256), device=f_ori.device) * 0).long()
        # layer_emb_c = self.lay_emb(layer_C)
        
        c_token = self.hyper_embeds[rc](f_ori)
        c_BLC = c_token

        result =  c_BLC.clone().unsqueeze(2)
        if result.shape[0] != x.shape[0]:
            result = result.repeat(x.shape[0], 1, 1, 1) 
...
@MODELS.register_module()
def PixArt_XL_C(**kwargs):
    model = PixArt_C_Ext(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
