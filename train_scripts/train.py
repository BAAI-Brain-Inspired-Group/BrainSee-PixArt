...
from diffusion.model.nets.PixArt_cross import inject_lora_and_freeze,LoRALinear
...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #     log_buffer.average()
            #     real_model = accelerator.unwrap_model(model) if 'accelerator' in globals() else model
            #     info = f"Step/Epoch [{(epoch-1)*len(train_dataloader)+step+1}/{epoch}][{step + 1}/{len(train_dataloader)}]:total_eta: {eta}, " \
            #         f"epoch_eta:{eta_epoch}, time_all:{t:.3f}, time_data:{t_d:.3f}, lr:{lr:.3e}, s:({real_model.h}, {real_model.w}), "

                # 计算损失并反向
                # with autocast(dtype=torch.bfloat32):
                loss_term = train_diffusion.training_losses(
                    model, clean_images, timesteps,
                    model_kwargs=dict(y=y, mask=y_mask, data_info=data_info, img_o=img_ori) #, img_o=img_ori
                )
                loss = loss_term['loss'].mean()
                accelerator.backward(loss)
                if accelerator.sync_gradients:
                ...
                # ---------- 日志聚合 ----------
                lr = lr_scheduler.get_last_lr()[0]
                loss_avg = accelerator.gather(loss).mean().item()           # ← 新增：聚合 loss
                logs = {args.loss_report_name: loss_avg}                    # ← 改：直接用 loss_avg
                if grad_norm is not None:
                    logs.update(grad_norm=accelerator.gather(grad_norm).mean().item())
                log_buffer.update(logs)
                ...
                if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
                    ...
                    avg_loss_interval = loss_accum / loss_count if loss_count > 0 else 0.0
                    loss_accum = 0.0
                    loss_count = 0                   
                    t = (time.time() - last_tic) / config.log_interval
                    t_d = data_time_all / config.log_interval
                    ...
                    eta_epoch = str(datetime.timedelta(seconds=int(
                        avg_time * (len(train_dataloader) - step - 1))))
                    ...
                    info = (
                        ...
                        f"lr:{lr:.3e}, loss:{avg_loss_interval:.4f}, "                     # ← 新增：打印 loss
                        f"s:({real_model.h}, {real_model.w}), "
                    )
                    logger.info(info)
        ...
        if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs:
            ...
            if accelerator.is_main_process:
                ...
                #     step=(epoch - 1) * len(train_dataloader) + step + 1,
                # )
                # logger.info(f"checkpoint path recorded at {ckpt_path}")

def parse_args():
    parser = argparse.ArgumentParser(description="Process some integers.")
    parser.add_argument("--config", type=str, help="config", 
     default='/home/easytaker/code/generate/dit-base/PixArt-alpha/configs/pixart_config/PixArt_xl2_img256_coco.py')
    parser.add_argument("--cloud", action='store_true', default=False, help="cloud or local machine")
    parser.add_argument('--work-dir', help='the dir to save logs and models', 
                        default='/home/easytaker/code/generate/dit-base/PixArt-alpha/output/debug/256_pieart_all_cross_lora_0')
...
def analyze_model_params(model, filter_key=None, save_path=None, verbose=True):
    """
    打印并统计模型中哪些参数是可训练的，哪些被冻结。

    Args:
        model (nn.Module): 你的 PyTorch 模型。
        filter_key (str, optional): 如果指定，只打印包含该关键字的参数。
        save_path (str, optional): 如果指定，保存输出到文件。
        verbose (bool): 是否打印到控制台。
    """
    lines = []
    total_params = 0
    trainable_params = 0
...
if __name__ == '__main__':
    ...
    model_ema = build_model(config.model,
                    config.grad_checkpointing,
                    config.get('fp32_attention', False),
                    input_size=latent_size,
                    learn_sigma=learn_sigma,
    ...
    inject_lora_and_freeze(model, keywords=("attn", "mlp"), r=8)
    inject_lora_and_freeze(model_ema, keywords=("attn", "mlp"), r=8)
