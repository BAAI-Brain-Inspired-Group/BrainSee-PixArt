...
from diffusion.model.nets import PixArt_XL_C, PixArt_XL_all_C
...
def get_args():
    ...
    parser.add_argument('--txt_file', default='/share/project/gcw-data/code/pixart/asset/samples_3.txt', type=str)
    parser.add_argument('--model_path', 
                        default='/share/project/gcw-data/code/pixart/output/txt-img/checkpoints/epoch_144_step_133200.pth', type=str)
...
def read_image_tensor_batched(
        ...
        dtype: torch.dtype = torch.float16      # ← 新增参数，默认 float16
    ) -> torch.Tensor:
    """
...
def visualize(items, bs, sample_steps, cfg_scale):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        with torch.no_grad():
            img_o = read_image_tensor_batched(path='/share/project/gcw-data/code/pixart/img_input/000000000030.jpg')
            img_o = img_o.cuda()
