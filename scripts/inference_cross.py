...
from diffusion.model.nets.PixArt_cross import inject_lora_and_freeze
...
def get_args():
    ...
    parser.add_argument('--t5_path', default='/home/easytaker', type=str)
    parser.add_argument('--tokenizer_path', default='/home/easytaker/sd-vae-ft-ema', type=str)
    parser.add_argument('--txt_file', default='/home/easytaker/code/generate/dit-base/PixArt-alpha/asset/samples_3.txt', type=str)
    parser.add_argument('--model_path', default='/home/easytaker/code/generate/dit-base/PixArt-alpha/output/debug/pieart_cross_lora_1/checkpoints/epoch_54_step_66582.pth', type=str)
    ...
    parser.add_argument('--save_name', default='/home/easytaker/code/generate/dit-base/PixArt-alpha/asset/test_sample', type=str)
...
def read_image_tensor_batched(
        path: str,
        img_size: int = 256,
        keep_ratio: bool = False,
        to_device: Optional[Union[str, torch.device]] = None,
        dtype: torch.dtype = torch.float32      # ← 新增参数，默认 float16
    ) -> torch.Tensor:
    """
    读取图像 → Tensor → 加 batch 维
    最终形状 [1, C, img_size, img_size]，并按 dtype 返回
    """
    img = Image.open(path).convert("RGB")

    if keep_ratio:
        tfm_resize = T.Compose([
            T.Resize(img_size, interpolation=InterpolationMode.BICUBIC),
            T.CenterCrop(img_size),
        ])
    else:
        tfm_resize = T.Resize((img_size, img_size), interpolation=InterpolationMode.BICUBIC)
    ...
    # 一步到位：同时指定 device 和 dtype
    if to_device is not None:
        tensor = tensor.to(device=to_device, dtype=dtype)
    else:
...
def visualize(items, bs, sample_steps, cfg_scale):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        with torch.no_grad():
            img_o = read_image_tensor_batched(path='/share/project/data/COCO2017/train2017/000000004979.jpg')
            img_o = img_o.cuda()
            ...
            if args.sampling_algo == 'iddpm':
                ...
                model_kwargs = dict(y=torch.cat([caption_embs, null_y]),
                                    cfg_scale=cfg_scale, data_info={'img_hw': hw, 'aspect_ratio': ar}, mask=emb_masks, img_o=img_o)
                ...
                model_kwargs = dict(data_info={'img_hw': hw, 'aspect_ratio': ar}, mask=emb_masks, img_o=img_o)
...
if __name__ == '__main__':
    ...
    if args.image_size == 256:
        model = PixArt_XL_C(input_size=latent_size, lewei_scale=lewei_scale[args.image_size]).to(device)
    ...
    # inject_lora_and_freeze(model, keywords=("attn", "mlp"), r=64)
